...
public class SceneSwitcherWindow : EditorWindow
{
    ...
    private void Update()
    {
        var currentBuildHash = GetBuildScenesHash();
    ...
    private void OnGUI()
    {
        ...
        showBuildSettingsScenes =
            EditorGUILayout.Foldout(showBuildSettingsScenes, $"Scenes in Build Settings ({buildScenes.Count})");
        ...
        if (GUILayout.Button("Clear other scenes")) ClearCustomScenes();
        if (showCustomScenes)
    ...
    [MenuItem("Sonat Framework/Tools/Scene Switcher")]
    ...
    {
        buildScenes = new List<string>();
        foreach (var scene in EditorBuildSettings.scenes) buildScenes.Add(scene.path);
    }

    private int GetBuildScenesHash()
    {
        var hash = 17;
        foreach (var scene in EditorBuildSettings.scenes) hash = hash * 31 + scene.path.GetHashCode();
        return hash;
    }
    ...
    private void DrawCustomListItems(Rect rect, int index, bool isActive, bool isFocused)
    {
        ...
        else if (GUI.Button(new Rect(rect.x, rect.y, rect.width - 145, EditorGUIUtility.singleLineHeight), sceneName) &&
            ...
            if (evt.type == EventType.DragPerform)
            {
                ...
                foreach (var draggedObject in DragAndDrop.objectReferences)
                {
                    ...
                    if (draggedObject is SceneAsset)
                        AddScene(path);
                    else if (Directory.Exists(path)) AddScenesFromFolder(path);
                }
            }
    ...
    private void LoadCustomScenes()
    {
        ...
        if (!string.IsNullOrEmpty(origin)) customScenes = JsonUtility.FromJson<SaveData>(origin).customScenesSaved;
    }

    private class SaveData
    {
        public readonly List<string> customScenesSaved;
