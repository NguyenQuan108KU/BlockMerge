...
public class LandingPipeline
{
    ...
    private readonly List<GameObject> _objsCache = new();
    private readonly List<Vector2Int> _coordsCache = new();
    private readonly HashSet<int> _affectedRowsCache = new();
    ...
    public LandingResult Execute(LandingContext ctx)
    {
        // 1. Clear caches
        _objsCache.Clear();
        _coordsCache.Clear();
    ...
    private void RegisterBlocksToGrid(LandingContext ctx)
    {
    ...
    private PostLandContext BuildPostLandContext(LandingContext ctx)
    {
    ...
    private SlideResult ExecuteSlide(LandingContext ctx)
    {
        var postCtx = BuildPostLandContext(ctx);
        return new SlideExecutor(ctx.Shape.slideDirection).Execute(postCtx);
    }

    private SlideResult ExecuteFloodFill(LandingContext ctx)
    {
        var postCtx = BuildPostLandContext(ctx);
