...
public class GridRowClearer
{
    ...
    private readonly List<int> _fullRowsCache = new();
    private readonly HashSet<int> _fullRowSetCache = new();
    private readonly HashSet<int> _impactedBlockIDsCache = new();
    private readonly List<GameObject> _visualsToDieCache = new();
    private readonly List<GameObject> _visualsToShakeCache = new();
    private readonly HashSet<int> _dieVisualIDsCache = new();
    private readonly HashSet<int> _shakeVisualIDsCache = new();
    private readonly HashSet<int> _strippingBlockIDsCache = new();
    private readonly HashSet<int> _blocksLostCellsCache = new();
    private readonly List<Vector2Int> _cellsToProcessCache = new();
    private readonly HashSet<int> _excludeRowSetCache = new();
    private readonly HashSet<int> _destroyedVisualIDsCache = new();

    private readonly HashSet<(int, int)> _seenCanonicalCache = new();
    private readonly List<Vector2Int> _canonicalCellsCache = new();
    private readonly List<HashSet<Vector2Int>> _componentsCache = new();
    private readonly HashSet<Vector2Int> _visitedCache = new();
    private readonly HashSet<Vector2Int> _cellSetCache = new();
    private readonly Queue<Vector2Int> _bfsQueueCache = new();

    private int[,] _virtualGrid;
    private readonly Dictionary<int, List<Vector2Int>> _virtualRegistryCache = new();
    private readonly Dictionary<int, int> _fallDistancesCache = new();
    private readonly Dictionary<int, int> _finalPositionsCache = new();
    private readonly List<(int x, int y, GameObject visual)> _pendingMovesCache = new();
    private readonly List<(int x, int y)> _cellsToClearCache = new();
    private readonly List<(int x, int y, int blockID)> _pendingWritesCache = new();
    private readonly HashSet<int> _animatedVisualIDsCache = new();
    private readonly List<(GameObject obj, Vector3 startPos, Vector3 endPos)> _animationsCache = new();
    private readonly List<int> _sortedBlocksCache = new();
    private readonly HashSet<int> _rowsToCheckCache = new();
    ...
    private static readonly Service<PoolingServiceAsync> _vfxPooling = new();
